...
class LabCategories extends PureComponent {
  ...
  onPressCatItem = async (type, value) => {
    ...
    this.props.navigation.navigate('LabSearchList', {
      inputDataFromLabCat: inputDataBySearch,
    });
  };
  ...
  filterCategories(searchValue) {
    ...
    if (!searchValue) {
      ...
      if (this.mainLabData != undefined) {
        const filteredCategories = this.mainLabData.filter(
          (ele) =>
            ele.lab_test_category_info.category_name
              .toLowerCase()
                  ...
                  <Col style={styles.mainCol}>
                    <TouchableOpacity
                      onPress={() =>
                        this.onPressCatItem(
                          'category_name',
                          item.lab_test_category_info.category_name,
                        )
                      }
                      style={{
                      ...
                      <FastImage
                        source={{
                          uri:
                            item.lab_test_category_info.category_image_url +
                            '/' +
                            item.lab_test_category_info.category_image_name,
                        }}
                        style={{
                          width: 60,
                          height: 60,
                      ...
                      <Row>
                        <Text style={styles.rsText}>
                          {' '}
                          {item.minPriceWithoutOffer != item.minPriceWithOffer
                            ? '₹' + item.minPriceWithoutOffer
                            : null}
                        </Text>
                        <Text style={styles.finalRs}>
                          ₹ {Math.round(item.minPriceWithOffer)}
                        </Text>
                      </Row>
                    </TouchableOpacity>
                  </Col>
