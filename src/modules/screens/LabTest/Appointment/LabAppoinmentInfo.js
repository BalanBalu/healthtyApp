...
import {
  updateLapAppointment,
  getLapTestPaymentDetails,
  getLabAppointmentById,
  getUserReviews,
} from '../../../providers/lab/lab.action';
...
class LabAppointmentInfo extends Component {
  ...
  initialFunction = async appointmentData => {
    ...
    if (
      ...
      appointmentData.is_review_added == undefined
    ) {
      this.setState({isAddReviewPopVisible: true});
    }
    const checkProposedNewTime = await AsyncStorage.getItem(this.appointmentId);
    if (
      appointmentData.appointment_status == 'PROPOSED_NEW_TIME' &&
      checkProposedNewTime != 'SKIP'
    ) {
  ...
  getAppointmentById = async appointmentId => {
    try {
      ...
      if (result.success) {
        ...
        const checkProposedNewTime = await AsyncStorage.getItem(
          this.appointmentId,
        );
        if (
          appointmentData.appointment_status == 'PROPOSED_NEW_TIME' &&
  ...
  getUserReport = async () => {
    try {
      ...
      let resultReport = await getUserRepportDetails(
        'labAppointment',
        userId,
        this.appointmentId,
      );
  ...
  getUserReviews = async () => {
    try {
      let reviewResult = await getUserReviews(
        'appointment',
        this.appointmentId,
      );
  ...
  async navigateCancelAppoointment() {
    try {
      ...
      this.props.navigation.navigate('LabCancelAppointment', {
  ...
  async navigateLabConfirmation() {
    try {
      ...
      this.props.navigation.navigate('labConfirmation', {
          ...
          <View style={{marginBottom: 20}}>
            <Card
              ...
              <CardItem header style={styles.cardItem}>
                <Grid>
                  <Text
                    style={{textAlign: 'right', fontSize: 14, marginTop: -15}}>
                    {'Ref no :' + data.token_no}
                  </Text>
                  <Row>
                    <Col style={{width: '25%'}}>
                      <TouchableOpacity
                        onPress={() =>
                          this.props.navigation.navigate('ImageView', {
                            passImage: renderLabProfileImage(
                              data.labInfo && data.labInfo,
                            ),
                            title: 'Profile photo',
                          })
                        }>
                      ...
                      <Row>
                        <Col size={9}>
                          <Text style={styles.Textname}>
                            {data.labInfo && data.labInfo.lab_name}
                          </Text>
                          <Text
                            style={{
                              fontSize: 12,
              ...
              <Grid>
                  ...
                  {data.appointment_status != undefined ? (
                    <Col size={3}>
                      <View style={{alignItems: 'center', marginLeft: -25}}>
                        <Icon
                          name={statusValue[data.appointment_status].icon}
                          style={{
                        ...
                        <Text
                          ...
                          {data.appointment_status == 'PAYMENT_IN_PROGRESS'
                            ? 'PAYMENT IN PROGRESS'
                ...
                {upcomingTap == 0 ? (
                  ...
                  this.state.appointmentStatus === 'APPROVED' ||
                      ...
                      <Col size={3}>
                        <Row style={{marginTop: 10}}>
                          <Button
                            style={[
                              styles.postponeButton,
                              {backgroundColor: primaryColor, marginLeft: 25},
                            ]}
                            onPress={() =>
            ...
            <Grid>
              <View style={{marginTop: 10}}>
                ...
                data.appointment_status === 'PROPOSED_NEW_TIME'
                  ? data.status_update_reason != undefined && (
                          ...
                          <Col style={{width: '92%', paddingTop: 5}}>
                            {data.appointment_status == 'PROPOSED_NEW_TIME' ? (
                              <Text style={styles.innerSubText1}>
                                {data.status_updated_by.toLowerCase() === 'user'
                                  ? 'Proposed a new time by You'
                                  : 'Rescheudled a new Time by Lab'}
                              </Text>
                            ) : null}
                            {data.appointment_status == 'CANCELED' ? (
                              <Text style={styles.innerSubText1}>
                            ...
                            <Text note style={styles.subTextInner1}>
                              {data.status_update_reason}
                            </Text>
                          </Col>
                        </Row>
                        {data.previous_data != undefined &&
                          data.appointment_status === 'PROPOSED_NEW_TIME' && (
                            <Row style={styles.rowSubText}>
                              <Col style={{width: '8%', paddingTop: 5}}>
                              ...
                              <Col style={{width: '92%', paddingTop: 5}}>
                                ...
                                <Text note style={styles.subTextInner1}>
                                  {formatDate(
                                    data.previous_data.startDateTime,
                                    'DD/MM/YYYY',
                                  )}
                                </Text>
                                <Text note style={styles.subTextInner1}>
                ...
                <Row
                  ...
                  <Col style={{width: '92%', paddingTop: 5}}>
                    ...
                    <Text
                      style={[
                        styles.commonText,
                        {color: '#4c4c4c', marginTop: 5},
                      ]}>
                      {data.labInfo && data.labInfo.lab_name}
                    </Text>
                    <Text note style={[styles.subTextInner1, {marginTop: 10}]}>
                      {(data.labInfo &&
                        data.labInfo.location &&
                        data.labInfo.location.address.no_and_street) +
                        ',' +
                        (data.labInfo &&
                          data.labInfo.location &&
                          data.labInfo.location.address.address_line_1) +
                        ',' +
                        (data.labInfo &&
                          data.labInfo.location &&
                          data.labInfo.location.address.district) +
                        ',' +
                        (data.labInfo &&
                          data.labInfo.location &&
                          data.labInfo.location.address.city) +
                        ',' +
                        (data.labInfo &&
                          data.labInfo.location &&
                          data.labInfo.location.address.state) +
                        ',' +
                        (data.labInfo &&
                          data.labInfo.location &&
                          data.labInfo.location.address.country) +
                        ',' +
                        (data.labInfo &&
                          data.labInfo.location &&
                          data.labInfo.location.address.pin_code)}
                    </Text>
                  </Col>
                </Row>
                ...
                {reviewData.length !== 0 ? (
                  <Row style={styles.rowSubText}>
                    ...
                    <Col style={{width: '92%', paddingTop: 5}}>
                      ...
                      />
                      <Text note style={styles.subTextInner1}>
                        {(reviewData[0] && reviewData[0].comments) || ''}
                      </Text>
                    </Col>
                  </Row>
                ) : reviewData.length == 0 ? (
                ...
                <Row style={styles.rowStyles}>
                  ...
                  <Col style={{width: '92%', paddingTop: 5}}>
                    ...
                    <Row style={{marginTop: 10}}>
                      ...
                      <Col style={{width: '25%'}}>
                        <Text note style={styles.downText}>
                          {'Rs.' +
                    ...
                    {paymentData.coupon_code_discount_amount ? (
                      <Row style={{marginTop: 10}}>
                        ...
                        <Col style={{width: '25%'}}>
                          <Text note style={styles.downText}>
                            {'Rs.' + paymentData.coupon_code_discount_amount}
                          </Text>
                        </Col>
                      </Row>
                    ) : null}
                    {paymentData.credit_point_discount_amount ? (
                      <Row style={{marginTop: 10}}>
                        ...
                        <Col style={{width: '25%'}}>
                          <Text note style={styles.downText}>
                            {'Rs.' + paymentData.credit_point_discount_amount}
                          </Text>
                        </Col>
                      </Row>
                    ) : null}
                    ...
                    <Row style={{marginTop: 10}}>
                      ...
                      <Col style={{width: '25%'}}>
                        <Text note style={styles.downText}>
                          {'Rs.' +
                            (paymentData.amount_due != undefined
                          ...
                          <Row>
                            ...
                            <Col size={3.4} style={{marginRight: 3}}>
                              <TouchableOpacity
                                ...
                                onPress={() =>
                                  this.updateLabAppointmentStatus(
                                    data,
                                    'APPROVED',
                                  )
                                }
                            ...
                            <Col size={3.6}>
                              <TouchableOpacity
                                ...
                                onPress={() =>
                                  this.navigateCancelAppoointment()
                                }
                                testID="cancelButton">
                                <Text
                                  style={{
