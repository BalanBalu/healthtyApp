...
class FilterDocInfo extends Component {
    ...
    async componentDidMount() {
        ...
        this.setState({ languageList, specialistInfoList });
    }
    ...
    render() {
        const { genderIndex, selectAvailabilityIndex, languages, genderSelected, selectedSpecialist,
            selectExperinceIndex, languageList, specialistInfoList } = this.state;
        return (
                        ...
                        <Row style={{ marginTop: 30, borderBottomWidth: 0, marginLeft: 10, marginRight: 10 }}>
                            <Col size={5} >
                                <TouchableOpacity
                                    ...
                                    <Text style={{ color: primaryColor, fontFamily: 'Roboto', fontSize: 13, textAlign: 'center',  }}>{translate("Clear Filters")}</Text>
                                </TouchableOpacity>
                            </Col>
                            <Col size={5} style={{ marginLeft: 20 }}>
                                <TouchableOpacity
                                    block success
                                    disabled={languages.length != 0 || genderSelected || selectedSpecialist.length != 0 || selectAvailabilityIndex != 0 || selectExperinceIndex != 0 ? false : true}
                                    style={languages.length != 0 || genderSelected || selectedSpecialist.length != 0 || selectAvailabilityIndex != 0 || selectExperinceIndex != 0 ? styles.viewDocButtonBgGreeen : styles.viewDocButtonBgGray}
                                    onPress={this.sendFilteredData}
                                >
                                    <Text style={languages.length != 0 || genderSelected || selectedSpecialist.length != 0 || selectAvailabilityIndex != 0 || selectExperinceIndex != 0 ? styles.enabledApplyTextColor : styles.defaultApplyTextColor}>{translate("Apply")}</Text>
                                </TouchableOpacity>
                            </Col>
                        </Row>
