...
class CancelService extends Component {
    ...
    getcancelList = async () => {
        try {
            ...
            this.setState({ cancelReasonList: cancelReasonList, type,isLoading: false })
        }
        catch (e) {
            console.log(e)
        }
    ...
    toggleRadio(data, index) {
        if(index!==this.state.cancelReasonList.length-1){
        this.setState({ selectedIndex: index,statusUpdateReason:data.cancel_reason })
        }else{
            this.setState({ selectedIndex: index,statusUpdateReason:'' })
        }
    ...
    cancelService = async () => {
        try {
            ...
            if (onlySpaceNotAllowed(this.state.statusUpdateReason) === true) {
                ...
                navigation.navigate({ routeName, key, params: { hasUpdateCancelService: true,statusUpdateReason:this.state.statusUpdateReason } });
        ...
        return (
          <Container>
            <Content style={{ padding: 30 }}>
              ...
              {selectedIndex === cancelReasonList.length-1 ?
